package org.example.springbootjobapprestcontrollerday11.repo;
...
public class JobRepo {
    ...
    public List<JobPost> getAllJobPostsFromDb() {
        return jobs;
    }

    public void addJobPostToDb(JobPost jobPost) {
        jobs.add(jobPost);
    }

    public void removeJobPostFromDb(int jobId) {
    ...
    public void updateJobPostToDb(JobPost jobPost,int jobId) {
        for(JobPost jobPostInDb : jobs)
        {
            if(jobPostInDb.getPostId() == jobId)
            {
                jobPostInDb.setPostId(jobPost.getPostId());
                jobPostInDb.setPostDesc(jobPost.getPostDesc());
                jobPostInDb.setPostTechStack(jobPost.getPostTechStack());
                jobPostInDb.setReqExperience(jobPost.getReqExperience());
                jobPostInDb.setPostProfile(jobPost.getPostProfile());
                return;
            }
        }
    }
